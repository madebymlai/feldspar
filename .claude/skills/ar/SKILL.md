---
name: ar
description: Opus-orchestrated adversarial review with cost/benefit analysis. Launches ar-o, ar-k, ar-mx, ar-glm in parallel, synthesizes findings. Use for validating design docs before /breakdown.
...
- **description**: Send a single message with four `Task` tool calls. Pass the document path from the `/ar` argument.
  ...
  **Spawn each teammate with these exact parameters:**
  ...
  | ar-mx | ar-mx | sonnet | bypassPermissions |
  | ar-glm | ar-glm | sonnet | bypassPermissions |
